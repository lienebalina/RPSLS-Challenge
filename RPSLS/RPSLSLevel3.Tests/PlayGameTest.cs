...
namespace RPSLSLevel3.Tests
{
    public class PlayGameTests
    {
        ...
        public void RunGame_RunWithValidInputs_PlayerWinsGame()
        {
            ...
            Assert.AreEqual("player wins this round!", lines[3]);
        }
        ...
        public void RunGame_RunWithValidInputs_ComputerWinsGame()
        {
            ...
            var playerMock = _mocker.GetMock<IPlayer>();
            playerMock.SetupSequence(p => p.TakePlayerInput())
                .Returns(2)
                .Returns(2);
            ...
            Assert.AreEqual($"{ComputerNames[0]} wins this round!", lines[3]);
        }

        [Test]
        public void ComputerVsComputer_ValidSettingsGiven_ValidGameplay()
        {
            var settings = new GameSettings(2, 1);
            ...
            //var randomMock = _mocker.GetMock<Random>();
            randomMock.SetupSequence(r => r.Next(1, 6))
                .Returns(3)
                .Returns(3);

            var randomInstance = randomMock.Object;

            var writer = new StringWriter();
            Console.SetOut(writer);

            _game.SetRandom(randomInstance);

            _game.ComputerVsComputer();

            var sb = writer.GetStringBuilder();
            var lines = sb.ToString().Split(Environment.NewLine, StringSplitOptions.TrimEntries);

            Assert.IsTrue(lines.Any(line => line.Contains($"{ComputerNames[0]} goes with Scissors")));
            Assert.IsTrue(lines.Any(line => line.Contains($"{ComputerNames[1]} goes with Scissors")));
        }
    }
}